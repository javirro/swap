...
export const splitTokenForSplitETHNoAdmin = async ({ provider, userAccount, chainId, from, tokenOutA, tokenOutB, amount }: SplitExactTokensForTokensParams) => {
  ...
  const currentAllowance: number = await tokenInContract.methods.allowance(userAccount, addresses.adminContract).call()
  ...
  const tx = await splitContract.methods.swapTokenForSplitETH(amountInWei, amountOutMinA, amountOutMinB, pathA, pathB, userAccount, deadline).send({
